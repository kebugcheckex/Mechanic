...
void TextDetector::WorkingThread()
{
    ...
    while (running)
    {
        m_pVideoReader->GetFrame(inputFrame);   // Get a frame from the video reader
        ...
        for (int step = 1; step < 3; step++)
        {
            ...
            for (int f = 0; f < NUM_FEATURES; f++)
            {
                ...
                Mat tmp_all_segmentations = Mat::zeros(240,320*11,CV_8UC3); // TODO hard-coded parameters
                drawClusters(tmp_segmentation, &regions, &meaningful_clusters);
                Mat tmp = Mat::zeros(240,320,CV_8UC3); // TODO hard-coded parameters
